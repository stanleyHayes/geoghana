...
| GEO-22.1 | EP-21 Backups & DR | 🟡 **Blocked on deployment** | Claude + Codex | Archive/restore tooling is reproducible and locally proven. **Cannot close from the codebase:** the acceptance criterion is Atlas *continuous backup with point-in-time restore, tested into a scratch cluster*. Verified independently — the configured Atlas cluster holds only `admin` and `local`, and `ghanageo` has **zero collections**, so there is nothing to back up. Enabling PITR is an Atlas project setting requiring org credentials. Unblocks the moment the dataset is deployed. |
| GEO-22.2 | EP-21 Backups & DR | 🟡 **Blocked on deployment** | Claude + Codex | `make restore-drill` passes end to end: a 78MB archive with a recorded SHA-256, restored into an isolated scratch database, **18/18 collections matching source counts exactly** (including 19,728 roads and 19,686 POIs) and 18/18 schema validators intact, with cleanup confirmed. **Cannot close:** measuring RPO/RTO needs a production recovery point, and production is empty. The drill itself is executed and documented; only the production leg remains. Evidence: `docs/runbooks/evidence/restore-drill-2026-08-29.md`. |
...
- [ ] Marketing website and documentation are public with quick starts for REST, GraphQL, gRPC and npm — all four are implemented on `/docs`, but none of the documented public hosts currently resolves in DNS. Deployment, TLS and live smoke/Core Web Vitals evidence remain required.
...
- [ ] Monitoring, alerting, backups and a completed restoration drill — `make restore-drill` passes end to end (78MB archive, 18/18 collections and validators matching, cleanup confirmed). Atlas continuous backup, measured RPO/RTO and production alerting need a deployment: the production cluster currently holds zero collections.
