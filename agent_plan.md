...
| GEO-4.7 | EP-04 Ingestion | 🟡 **Partial** | Claude | Geofabrik Ghana extract ingested: 19,728 named roads, 19,686 POIs, 9,406 settlements. ODbL attribution enforced by the domain validator, the Mongo schema, every API response and every export file (file- and feature-level). 98% of POIs assigned to a district by containment. **Remaining:** roads carry no region/district — linestring containment is not the same problem as point-in-polygon and needs its own pass; settlements are scanned but not imported (places come from `data import`); and the extract does not supply the 13 missing district boundaries, which need an official GSS release. |
