...
| GEO-3.1 | EP-03 Schema | 🟡 **Partial** | Claude / Codex audit | Collections, strict JSON Schema validators, 2dsphere and compound indexes are live. R7 is not satisfied: the current canonical `_id` values are deterministic slugs (`gh-region-*`, `gh-district-*`, `gh-place-*`) while the plan and OpenAPI contract require ULIDs. Closing this requires a deterministic, relationship-safe ID migration plus redirects for every already-published legacy ID; it must not be treated as a cosmetic validator change. |
...
| GEO-15.4 | EP-15 Sandbox | 🟡 **Partial** | Codex (L9) | Allow-listed, 16 KiB-capped and 30 rpm gRPC bridge executes anonymous reads without privileged credentials; method picker generates typed per-method forms. Live `Search`/`Nearby` and enforced HTTP 429 pass. Streaming remains blocked by the still-unimplemented GEO-8.3 change feed. |
...
| GEO-15.6 | EP-15 Sandbox | 🟡 **Partial** | Codex (L9) | One-click Osu, Adenta, Tema Community 25 and Kumasi samples exist; account CTA serializes protocol, request and payload. Portal signup/replay consumption remains blocked on GEO-9.2/GEO-16 account flows. |
...
| GEO-16.2–16.4 | EP-16 Portal | 🟡 **Partial** | Codex (L4/L10; coordinated L0 wiring) | Members/roles, one-time invitations, revocation, acceptance and owner-only transfer now work end to end. Applications persist test/live environments, domains, callback metadata and the fixed free plan. Keys expose class/scope, expiry, IP/origin restrictions, copy-confirm, rotation and revocation. Full API tests, portal typecheck/build and a live two-account ownership-transfer lifecycle pass; replayed session cookies and former-owner transfer are denied. Remaining completion gates are the L0-owned published REST contract/docs plus the full three-material/theme/viewport UI matrix. |
| GEO-16.5–16.6 | EP-16 Portal | 🟡 **Partial** | Codex (L4/L10; coordinated L0 wiring) | Durable, secret-free REST/GraphQL/gRPC telemetry now records request ID, normalized operation, status/error, latency, quota charge/remaining and geography against the verified key's organization/application. Mongo enforces a strict schema, tenant/time indexes and 30-day TTL. Member-authorized summaries and 25-row cursor pages drive token-themed Recharts, metrics, skeleton/empty/error states and keyboard-scrollable logs. Full API tests/vet, portal typecheck/build, live three-protocol aggregation, no-overlap pagination, outsider 404, secret-field absence and 390px no-page-overflow checks pass. Remaining completion gates are configurable enterprise retention, L0-owned REST contract/docs and the full design matrix. |
...
| GEO-4.6 | EP-04 Ingestion | 🟡 **Partial** | Claude | geoBoundaries ADM2 ingested: 16/16 regions and 248/261 districts. Matching is now gated on region GEOMETRICALLY — a boundary can only match a district in the region its own shape sits inside, so "Bolgatanga East" (Upper East) can no longer reach "Ga East" (Greater Accra). The remaining 13 are a genuine 2019-vs-2026 structural gap, documented per district in `docs/boundary-coverage.md`. Closing it needs an official GSS release; no matcher improvement can. |
| GEO-4.7 | EP-04 Ingestion | ⬜ Next | unassigned | OpenStreetMap (Geofabrik Ghana extract) for settlements, roads and POIs, with ODbL attribution. Would also supply the 13 missing district boundaries. |
...
- **Partial dependency track:** GEO-9.2 and the complete runtime developer journey now pass, including passkeys, invitations, ownership transfer, application metadata, expiry controls and attributed usage inspection across REST/GraphQL/gRPC. Published REST contract/docs and the remaining full design matrix still block story-level completion.
...
- **Data contract correction:** GEO-3.1 was overstated as Done. Storage/schema/index work is complete, but R7's ULID requirement conflicts with the live slug identifiers and the published OpenAPI description. A compatibility-preserving migration and legacy redirects are required before the story can close.
...
- [ ] npm package published with TypeScript types and an explicit dataset version — `@ghanageo/client` and `@ghanageo/react` build and typecheck; **not yet published to npm**, which is a release action rather than code.
...
- [x] Marketing website and documentation are public with quick starts for REST, GraphQL, gRPC and npm — all four on `/docs`.
...
- [ ] Monitoring, alerting, backups and a completed restoration drill — local drill passed and is documented; Atlas continuous backup, measured RPO/RTO and production alerting remain (GEO-22.1–22.2).
