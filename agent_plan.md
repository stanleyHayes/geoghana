...
**Status:** V1 implementation and local verification are substantially complete, and the public launch boundary is now live: four Vercel frontends plus the public REST/GraphQL API at `api-geo.digitalghana.dev`, backed by MongoDB Atlas, Render Key Value and a self-hosted Typesense node (2026-09-12). Stable is not claimed. The remaining critical path is the red SDK release-conformance matrix in CI, persistent storage for search and exports, native gRPC, and observability/alerting. Evidence: `docs/runbooks/evidence/api-production-launch-2026-09-12.md`.
...
**2026-09-12 — GEO-30.1 API production.** The public REST/GraphQL API is live at `api-geo.digitalghana.dev`. Five defects were fixed to get there: the container image could never build (`go.work` declares `./cli`, which `.dockerignore` excludes); the API fail-closed on absent Resend configuration; `MONGO_URI` was stored with its surrounding quotes so the driver rejected the scheme; `API_TRUSTED_PROXY_CIDRS` was unset while proxy headers were trusted; and the worker inherited the API entrypoint. `/health`, `/v1/regions`, `/v1/districts`, `/graphql` and `/v1/search` all answer 200, with 16,201 documents indexed. Stable is not claimed. Evidence: `docs/runbooks/evidence/api-production-launch-2026-09-12.md`.

**2026-09-01 — GEO-30.1 frontend production.** Marketing/docs, sandbox, developer console and admin are independently `READY` on Vercel and attached to `geo`, `sandbox-geo`, `console-geo` and `admin-geo.digitalghana.dev`. All representative routes return HTTP 200 over a valid `*.digitalghana.dev` certificate. The frontend boundary is live; product/API launch is still blocked by the 14 real production-preflight provider values and absent Render services. Evidence and immutable deployment IDs are recorded in `docs/runbooks/evidence/digitalghana-launch-reconciliation-2026-09-01.md`.

| Story                       | Epic                           | Status                         | Agent                                        | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --------------------------- | ------------------------------ | ------------------------------ | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GEO-30.1                    | EP-30 Digital Ghana Launch     | ✅ **Done — stable gates remain** | Codex (L0/L8–L12)                         | Four Vercel surfaces READY; API `srv-daiidlnqj5pc73a8esb0` (deploy `dep-daij5srm8hqs73d36ajg`), worker `srv-daiif2u7bikc738prt50` and private Typesense `srv-daiiji1594qs738tpot0` all live on Render with Key Value `red-daiicmvqj5pc73a8b7lg`. `api-geo.digitalghana.dev` returns 200 over a valid Google Trust Services certificate; 16,201 documents indexed and search returns scored results. Stable gates remain: red SDK conformance CI, no persistent disk for search/exports, gRPC undeployed, observability and alerting unset. Evidence: `docs/runbooks/evidence/api-production-launch-2026-09-12.md`. |
...
| GEO-18.12                   | EP-18 Marketing & Docs         | ✅ **Done (code)**             | Codex (L8/L12)                               | All 11 public routes now emit unique canonical, description, Open Graph and Twitter metadata; Organization/WebSite/SoftwareApplication JSON-LD includes the required free Offer; environment-aware robots/noindex and sitemap behavior, manifest, security headers and an executable dual-mode SEO audit are in place. Production audit: SEO 100, accessibility 100, best practices 100, performance 93, CLS 0; throttled lab LCP is 3.2s. Public DNS, Search Console and field Core Web Vitals remain external launch gates. Evidence: `docs/runbooks/evidence/seo-production-readiness-2026-08-30.md`.                                                                                                                                                                                                                        |
...
| GEO-15.SCROLL               | EP-15 Sandbox                  | ✅ **Done**                    | Codex (L9)                                   | Desktop request library and workbench are independently scrollable, focusable regions inside a viewport-locked shell; header/footer stay fixed. Compact layouts return to one natural document scroll. At 1440×700 both panes overflow and retain distinct scroll positions while document height stays 700px; at 390×700 both panes are `overflow: visible` and document height is 2143px. Sandbox typecheck/build and diff check pass.                                                                                                                                                                                                                                                                                                                                                                                        |
...
| GEO-17.3                    | EP-17 Admin                    | ✅ **Done**                    | Codex takeover (L2/L3/L11)                   | Atomic idempotent create/edit/deprecate for regions, districts and places; durable redirects; alias lifecycle; road/POI create/deprecate; RBAC/MFA; hash-chained audit; and real operator controls. Region/district boundary CAS and validation are live. Full road/POI PATCH editing stays in the Post-V1 expansion scope.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| GEO-17.ROUTES               | EP-17 Admin                    | ✅ **Done**                    | Codex (L11)                                  | The public operator IA now exposes 45 real routes plus two implicit auth entries. Every former `PlannedScreen` destination was removed from the navigation and route tree instead of shipping product briefs as features. Branded authenticated 404/loading states remain; the repository link gate reports zero dead or orphan routes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| GEO-17.AUTH-PRODUCTION      | EP-17 Admin / EP-20 Security   | ✅ **Done (code)**             | Codex takeover (L11/L4/L12)                  | All admin routes fail closed behind API-validated sessions; non-admin roles are denied before render, privileged actions remain API-RBAC/MFA enforced, expiry/logout/rotation/recovery/MFA enrolment are real, and CSP/HSTS/private-no-store/immutable-asset behavior is runtime-proven. WebAuthn bodies and Argon2 concurrency are bounded; auth throttling fails closed independently of Redis; audit and publication invariants are database-enforced across replicas. Independent security re-review found no critical/high code blocker. Public deployment remains externally blocked. Evidence: `docs/runbooks/evidence/admin-production-takeover-2026-08-30.md`.                                                                                                                                                         |
| GEO-17.AUTH-POLISH          | EP-17 Admin / EP-09 Identity   | ✅ **Done**                    | Codex (L4/L10/L11)                           | Idempotent operator bootstrap creates or refreshes a verified local `SUPER_ADMIN` without persisting plaintext credentials and revokes old sessions on refresh. Admin and developer auth/recovery forms now use branded icon fields, useful placeholders and accessible password visibility controls; admin login links to the existing anti-enumeration-safe reset flow. The requested local account was seeded. Full API tests, admin/portal typechecks, admin tests, both production builds, live toggle/link/overflow checks and diff check pass.                                                                                                                                                                                                                                                                           |
| GEO-17.MFA-ENROLMENT        | EP-17 Admin / EP-09 Identity   | ✅ **Done**                    | Codex (L4/L11)                               | TOTP enrolment now renders the `otpauth://` secret as an on-device QR code, exposes the manual key, and offers the ten one-time recovery codes as a local text download. The accessible six-cell OTP control supports numeric keyboards, paste, arrows, backspace and per-digit labels. An explicit operator-only `--reset-mfa` bootstrap recovery clears an interrupted local enrolment and revokes sessions; the requested account was left reset and ready to enrol. Full API tests, admin typecheck/lint/tests/build, live QR/recovery/OTP/overflow checks and diff check pass.                                                                                                                                                                                                                                             |
| GEO-17.APPS-NULL-SAFETY     | EP-17 Admin                    | ✅ **Done**                    | Codex (L3/L11)                               | `/developers/apps` no longer crashes when legacy application records contain null optional lists. The REST DTO now emits stable empty arrays, the client remains defensive during rolling deploys, and a regression test locks the contract. Full API tests, admin typecheck/lint/tests/build, live four-row render, fallback text, zero console errors/overflow and diff check pass.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| GEO-17.TABLE-ACTIONS        | EP-17 Admin                    | ✅ **Done**                    | Codex (L11)                                  | Every admin data table now exposes a compact, branded icon-action rail with tooltips and accessible names. Geography rows provide view/edit/lifecycle routes; developer, import, release, moderation and evidence rows expose safe inspect/manage actions; immutable rows use truthful copy/download actions instead of invented mutations. Admin typecheck, lint, tests, production build, diff check and authenticated live QA pass.                                                                                                                                                                                                                                                                                                                                                                                          |
| GEO-17.NAVBAR-DECLUTTER     | EP-17 Admin                    | ✅ **Done**                    | Codex (L11)                                  | The admin toolbar now contains navigation utilities only: the dead global Create and app-switch affordances are gone, breadcrumbs lead the page canvas, and environment/current-release context sits quietly in the sidebar footer with a real release link. Route-aware detail breadcrumbs, admin typecheck/lint/tests/build, authenticated live QA and diff check pass.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| GEO-17.HEADER-DATA-REPAIR   | EP-17 Admin                    | ✅ **Done**                    | Codex (L11)                                  | Page-header actions stay pinned to the far-right grid column on desktop and move beneath the description only at the 640px mobile breakpoint. All public admin reads now use the stable same-origin `/api/v1` proxy instead of fragile direct browser calls to port 8180. Regions (16), Districts (25/page) and Places (25/page) render live without fetch errors; typecheck/lint/tests/build and diff check pass.                                                                                                                                                                                                                                                                                                                                                                                                              |
| GEO-17.SIDEBAR-CONNECTORS   | EP-17 Admin                    | ✅ **Done**                    | Codex (L11)                                  | Every expanded multi-item sidebar group now has a visible tree trunk and branch connectors. The active route branch uses the brand colour and soft focus halo; connectors remain available in the mobile drawer and disappear cleanly in collapsed desktop mode. Live expanded/collapsed QA, typecheck/lint/tests/build and diff check pass.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| GEO-17.TABLE-ACTION-STYLING | EP-17 Admin                    | ✅ **Done**                    | Codex (L11)                                  | Row actions now sit in a recessed borderless tray with clearly raised, brand-tinted neu/clay icon buttons. Hover, pressed, focus, disabled and destructive states have distinct depth and colour while retaining tooltips and accessible labels. Live 16-row visual QA, typecheck/lint/tests/build and diff check pass.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| GEO-14.CUSTOM-CONTROLS      | EP-14 Design / EP-17 Admin     | ✅ **Done**                    | Codex (L7/L11)                               | Added the shared animated EmptyState and applied it to async, explorer, review, developer, geography, operations, and ingest surfaces; rebuilt Geometry Workbench as a responsive branded instrument panel; replaced remaining native selects and datetime-local controls with shared Radix/date-time primitives. Evidence: source scan reports zero rendered native select/checkbox/radio/date/time controls; browser QA confirmed the Geometry Workbench selector and empty state; `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm build`, and `git diff --check` pass. |
| GEO-22.RENDER-READINESS     | EP-21 Backups / EP-23 Launch   | ✅ **Done (code)**             | Codex (L12)                                  | `render.yaml` now provisions a private persistent Frankfurt Render Key Value service and injects its internal connection string into API/worker; production files contain deterministic canonical origins, HTTPS, passkey, and service-mode settings. Blueprint/preflight regression tests pass. The fail-closed deployment gate is reduced from 28 to 14 backend entries (seven unique provider/network inputs) and each frontend now lacks only its public Sentry DSN. |
| GEO-17.DATA-COMPLETION      | EP-17 Admin / EP-22 Acceptance | ✅ **Done (code)**             | Codex takeover (L11/L2/L3/L4/L12)            | Dashboard, operations, imports, moderation, release lifecycle/changelog/publish/rollback, developer support, fair-use, health, geography/geometry, aliases, redirects and audit export now use real protected contracts with truthful unavailable states where infrastructure does not expose deeper internals. No `PlannedScreen` route remains. Local functional/security/build gates pass; public deployment remains blocked only by the external launch gates recorded in `docs/runbooks/evidence/public-readiness-final-2026-08-30.md`.                                                                                                                                                                                                                                                                                    |
| GEO-17.READINESS            | EP-17 Admin                    | ✅ **Superseded**              | Codex (L11)                                  | The temporary operator-brief phase was removed. Unimplemented Post-V1 destinations are no longer presented as routes; the V1 navigation contains only functional surfaces.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| GEO-22.1                    | EP-21 Backups & DR             | 🟡 **Blocked on cluster tier** | Claude + Codex                               | Production is now deployed and seeded, and the archive/restore path is proven end to end against it (GEO-22.2). **Cannot close:** the criterion is Atlas _continuous backup with point-in-time restore_, and this cluster is **M0 (shared/free)**, where those features do not exist — they begin at M10. Verified directly: admin commands return AtlasError. Until the cluster is upgraded, RPO equals the age of the last manual `mongodump`. This is a billing decision, not a code or tooling gap.                                                                                                                                                                                                                                                                                                                         |
...
| **EP-27** | **GhanaPostGPS Adapter** — _gated on a signed licence, no date_                          | L1     | **gated** | §28, R1                      |
...
**Golden cases (Spec §10):** `"tema comm 25"` → _Tema Community 25_; `"kwabena"` → candidates including _Kwabenya_; `"circle"` → locality/POI candidates ranked with Accra context.
...
**Acceptance criteria (V1 release scope):** Create/edit/deprecate regions, districts and places; manage place aliases; keep roads and POIs as provenance-rich read catalogues with idempotent create/deprecate controls. Full road/POI editing remains Post-V1 expansion per §23. Every mutation is permission-scoped and writes an audit row (GEO-9.7). Deprecation creates a redirect, never a hard delete (GEO-3.3).

#### GEO-17.4 — Geometry editor

**Depends-On:** GEO-5.3, GEO-17.2 · **Spec:** §17
**Acceptance criteria:** Edit Polygon/MultiPolygon GeoJSON in a structured workbench with **validation before publish** — client-side coordinate/ring checks plus authoritative server self-intersection, containment and coordinate-range validation block the save with an explanatory error. Strong ETag/`If-Match` compare-and-swap prevents stale overwrites. Interactive vertex drawing is Post-V1 polish, not a V1 correctness gate.

#### GEO-17.5 — Import jobs & source runs

**Depends-On:** GEO-4.4 · **Spec:** §17
**Acceptance criteria:** Job list with status, duration, records processed, errors, reconciliation conflicts and duplicate candidates. Drill-down exposes the privacy-safe source record reference and payload hash. Raw provider payload bytes are deliberately not retained in the operator database because they can contain licensed or personal source material; retrieving them requires an adapter-specific lawful source workflow.
...
- [x] Admin can review, import, edit, publish and roll back a dataset version with complete audit history — durable imports expose privacy-safe record/hash evidence; moderation has audited CAS transitions and diffs; geography writes are transactional/idempotent with redirects and aliases; release notes and `draft → validation → review → approved → published → rollback` transitions are real, permission-gated and audited; publish verifies artifact bytes and uses exact-version confirmation.
...
| **V2.X** | _Gated_      | **Licensed GhanaPostGPS adapter**                                               | **Signed licence or written integration agreement. This is a legal gate, not an engineering one — it has no target date and must never be "started early".** |
...
**Delivery status (2026-08-30):** ✅ **Implementation and local verification complete.** The approved V2.0 SDK program now includes the TypeScript reference family, Python, Go, Dart core plus Flutter companion, Java plus Spring starter, .NET and PHP plus Laravel integration; strict language-neutral REST/GraphQL/native-gRPC conformance; coordinated, resumable multi-registry release automation; and nine-target compiled documentation parity. The retained 13-report matrix proves 109 distinct case/protocol evidence pairs with zero failures/skips. Public registry ownership/credentials, protected-environment approvals, the V1 public-launch gate and the frozen-release-cycle requirement remain external publication gates; no registry publication is claimed.

#### GEO-24.1 — SDK conformance suite (build this first)

**Status:** ✅ **Done** · **Agent:** Codex (L0/L6) · **Claimed/verified:** 2026-08-29
...
- A reusable runner contract, deterministic test-server orchestration and the TypeScript reference adapter execute the suite through public SDK operations. This bootstrap gate unblocks the language stories.
- **CI runs every available SDK runner on every contract change.** A contract PR that breaks an implemented SDK fails before merge.
...
#### GEO-24.1M — Complete conformance matrix (closes before release)

**Status:** ✅ **Done** · **Agent:** Codex (L0/L6) · **Verified:** 2026-08-30 · **Depends-On:** GEO-24.3–24.8 · **Blocks:** GEO-24.9
**Acceptance criteria:** Every TypeScript and non-TypeScript SDK ships its thin live-test-server runner; CI executes the complete language/protocol matrix on contract changes; required failures, skips or missing cases fail the release gate. This accumulating gate separates the bootstrap dependency from the final all-SDK proof and removes the circular requirement that previously made `GEO-24.1` both depend on and block future SDK runners.

#### GEO-24.2 — SDK design charter

**Status:** ✅ **Done** · **Agent:** Codex (L6) · **Verified:** 2026-08-29
**Depends-On:** GEO-24.1
...
**Status:** ✅ **Done** · **Agent:** Codex (L6) · **Claimed/verified:** 2026-08-29
...
| ------------ | --------------------------------- | ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **GEO-24.3** | `ghanageo` (Python)               | PyPI          | ✅ **Done — Codex (L6), verified 2026-08-30.** Sync **and** `async`/`httpx` clients; full type hints + `py.typed`; Pydantic result models; pandas/GeoPandas interop helper for dataset downloads; supports the 3 most recent CPython releases |
| **GEO-24.4** | `github.com/ghanageo/ghanageo-go` | Go modules    | ✅ **Done — Codex (L6), verified 2026-08-30.** `context.Context` first arg everywhere; functional options; errors via `errors.Is`/`As` on typed sentinels; generated from the same protobufs as the server                                    |
| **GEO-24.5** | `ghanageo` (Dart/Flutter)         | pub.dev       | ✅ **Done — Codex (L6), verified 2026-08-30.** Null-safe; `Future`/`Stream` APIs; `CancelToken`; a `GhanaGeoLocationPicker` widget mirroring the React `LocationCombobox`; offline region datasets (ties to GEO-26.3)                         |
| **GEO-24.6** | `dev.ghanageo:ghanageo-java`      | Maven Central | ✅ **Done — Codex (L6), verified 2026-08-30.** Java 21+; immutable records; `CompletableFuture` async; a Spring Boot starter with auto-configuration                                                                                          |
| **GEO-24.7** | `GhanaGeo` (.NET)                 | NuGet         | ✅ **Done — Codex (L6), verified 2026-08-30.** `IGhanaGeoClient` for DI; `IAsyncEnumerable<T>` pagination; `CancellationToken` throughout; nullable reference types enabled                                                                   |
| **GEO-24.8** | `ghanageo/ghanageo-php`           | Packagist     | ✅ **Done — Codex (L6), verified 2026-08-30.** PSR-18 HTTP client, PSR-7 messages, PSR-3 logging; a Laravel service provider and facade                                                                                                       |

#### GEO-24.9 — SDK release automation

**Status:** ✅ **Done (automation; publication externally gated)** · **Agent:** Codex (L0/L6) · **Verified:** 2026-08-30
**Depends-On:** GEO-24.1M, GEO-24.3–24.8
...
**Status:** ✅ **Done** · **Agent:** Codex (L6/L8) · **Verified:** 2026-08-30
**Depends-On:** GEO-24.9
...
### 23.7 What v2 explicitly does _not_ include
...
- Redistribution of any GhanaPostGPS digital address, licensed or not — the licence in EP-27 permits _lookup_, never _redistribution_ (R1).
...
- An explicit line: _"Donating does not change your rate limits."_
...
_End of plan. Update §1a after every batch of work._
