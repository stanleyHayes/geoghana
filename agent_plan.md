...
| GEO-16.2–16.4 | EP-16 Portal | 🟡 **Partial** | Codex (L10) | The real signed-in portal can create/select organizations and applications and create/copy-confirm/rotate/revoke restricted keys. Browser-key unsafe combinations are blocked/explained. Portal typecheck/build and live Playwright lifecycle pass with zero axe violations or overflow at 390/1440px. Organization members/invitations/ownership transfer, application domain/callback metadata, and the expiry control remain before these three stories close. |
...
- **Partial dependency track:** GEO-9.2 now has verified email/password, TOTP/recovery and rotating session flows. Passkeys/WebAuthn plus the organization/application/key-management and usage surfaces still block the complete developer onboarding checklist.
