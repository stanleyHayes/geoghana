...
# read_existing <file> <KEY> — echoes the current value, or empty.
read_existing() {
  [[ -f "$1" ]] || return 0
...
"MONGO_URI=PASTE_MONGODB_ATLAS_URI" \
"MONGO_DB=ghanageo" \
"" \
"# ── PASTE: managed Redis (Upstash, Redis Cloud, or Render Key Value) ────" \
"REDIS_URL=PASTE_REDIS_URL" \
"" \
"# ── PASTE: Typesense Cloud, or your self-hosted node ────────────────────" \
"TYPESENSE_URL=PASTE_TYPESENSE_URL" \
"TYPESENSE_API_KEY=PASTE_TYPESENSE_API_KEY" \
"" \
...
"NEXT_PUBLIC_GHANAGEO_BROWSER_KEY=" \
"" \
...
"NEXT_PUBLIC_GHANAGEO_BROWSER_KEY=PASTE_BROWSER_KEY" \
"" \
"# ── PASTE: analytics and error reporting (optional) ──" \
"NEXT_PUBLIC_SENTRY_DSN=PASTE_SENTRY_DSN_OR_LEAVE_BLANK" \
"" \
"# --- Server-only: never prefixed NEXT_PUBLIC_. ---" \
"AUTH_SECRET=$AUTH_SECRET" \
"SESSION_SECRET=$SESSION_SECRET" \
"# ── PASTE: a SERVER-class key. This must never reach a client bundle. ──" \
"GHANAGEO_SERVER_KEY=PASTE_SERVER_KEY" \
"INTERNAL_SERVICE_TOKEN=$INTERNAL_SERVICE_TOKEN"
}
