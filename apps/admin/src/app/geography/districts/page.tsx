...
import {
  fetchAll, listRegionDistricts, listDistricts, listRegions,
  type District, type Page, type Region,
} from "@/lib/api";
        ...
        {(page) => {
          ...
          <>
            <p style={{ color: "var(--fg-muted)", fontSize: "var(--text-sm)",
              ...
              {!page.complete ? (
                <strong style={{ color: "var(--warning, var(--danger))" }}> · truncated, more pages exist</strong>
              ) : null}
            </p>
                  ...
                  <tbody>
                    {visibleDistricts.map((d) => (
                      <tr key={d.id}>
                        ...
                        <td style={{ color: d.type === "UNSPECIFIED" ? "var(--fg-subtle)" : undefined }}>
                          {d.type === "UNSPECIFIED" ? "not recorded" : d.type}
                        </td>
                        <td><VerificationBadge status={d.verificationStatus} /></td>
                      </tr>
                    ))}
                  </tbody>
                </table>
