...
import { sandboxOrigin, webOrigin } from "@/lib/public-origins";

const tools = [
  { icon: Terminal, label: "REST API", meta: "11 endpoints", href: sandboxOrigin },
  { icon: Braces, label: "GraphQL", meta: "Schema ready", href: sandboxOrigin },
  { icon: Code2, label: "SDKs & CLI", meta: "7 platforms", href: `${webOrigin}/docs` },
];
const activity = [
  ...
  return (
    <div className="portal-shell">
      ...
      <header className="portal-header">
        <a className="portal-brand" href={webOrigin} aria-label="GhanaGeo home"><span className="portal-brand__mark"><MapPin size={17} /></span><span><strong>GhanaGeo</strong><small>Developer portal</small></span></a>
        <nav className="portal-nav" aria-label="Portal navigation"><a href="#overview" aria-current="page">Overview</a><a href={sandboxOrigin}>API sandbox</a><a href={`${webOrigin}/docs`}>Documentation</a></nav>
        ...
        {menuOpen ? <nav className="portal-mobile-nav"><a href="#overview">Overview</a><a href={sandboxOrigin}>API sandbox</a><a href={`${webOrigin}/docs`}>Documentation</a></nav> : null}
      </header>

      <main id="main" className="portal-main">
        <section className="portal-hero" id="overview">
          <div><p className="portal-eyebrow"><Sparkles size={14} /> Public infrastructure, developer ready</p><h1>Build with Ghana&rsquo;s geography.</h1><p className="portal-hero__copy">Search places, resolve coordinates and move through Ghana&rsquo;s administrative structure with one dependable dataset.</p><div className="portal-hero__actions"><a className="portal-primary" href={sandboxOrigin}>Make your first request <ArrowRight size={16} /></a><a className="portal-secondary" href={`${webOrigin}/docs`}><BookOpen size={16} /> Read the docs</a></div></div>
        ...
        <section className="portal-bottom-grid"><a href={sandboxOrigin}><Search size={19} /><span><strong>Explore the dataset</strong><small>Run search, reverse geocoding and boundary requests.</small></span><ArrowRight size={17} /></a><a href={`${webOrigin}/transparency`}><Activity size={19} /><span><strong>Service transparency</strong><small>See operating commitments, reporting and provenance.</small></span><ArrowRight size={17} /></a></section>
        <AccountWorkspace />
      </main>
